...
public class PersonService {
     ...
     * @throws PersonNotFoundException if person is not founded
     */
    public PersonData findById(String idString){
        ObjectId id = new ObjectId(idString);
        var p= repository.findById(id);
        if(p.isEmpty()) throw new PersonNotFoundException(idString);
        return p.get();
    }
